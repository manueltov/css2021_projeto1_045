...
public class EventTypeCatalog {
	...
	public EventType getEventTypeByName(String eventType) throws EventTypeNotFoundException {
		try {
			...
			throw new EventTypeNotFoundException("Event Type with name " + eventType + " does not exist", e);
		}
	}

	public Iterable<EventType> getEventTypes() {
		try {
			TypedQuery<EventType> query = em.createNamedQuery(EventType.GET_ALL_EVENT_TYPES, EventType.class);
